...
def render_dashboard(database_url: str | None = None):
    ...
    b1, b2 = st.columns(2)
    b1.markdown(f"**Trading mode:** {view['trading_mode']}")
    b1.markdown(f"**Review required:** {view['review_required']}")
    b1.markdown(f"**Latest stop reason:** {view['latest_stop_reason']}")
    b2.markdown(f"**Latest safe error message:** {view['latest_safe_error_message']}")
    b2.markdown(f"**Last run timestamp:** {view['last_run_timestamp']}")
    b2.markdown(f"**Last successful run timestamp:** {view['last_successful_run_timestamp']}")

    st.header("Market and Signal")
    m1, m2, m3, m4 = st.columns(4)
    m1.markdown(f"**Market open/closed:** {view['market_status']['label']}")
    m1.markdown(f"**Latest SPY price:** {view['latest_spy_price']}")
    m2.markdown(f"**Latest market-data timestamp:** {view['latest_market_data_timestamp']}")
    m2.markdown(f"**Short moving average:** {view['short_moving_average']}")
    m3.markdown(f"**Long moving average:** {view['long_moving_average']}")
    m3.markdown(f"**Generated signal:** {view['generated_signal']}")
    m4.markdown(f"**Trade or skip reason:** {view['trade_or_skip_reason']}")
    ...
    s1.markdown(f"**Cooldown:** {view['cooldown_status']}")
    s2.markdown(f"**Duplicate signal:** {view['duplicate_signal_status']}")
    s3.markdown(f"**Pending order:** {view['pending_order_status']}")
    s4.markdown(f"**Daily loss stop:** {view['daily_loss_stop_status']}")
